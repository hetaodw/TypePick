...
The `.env` is not sent to CI. The actual Jev endpoint is separately exercised locally with synthetic text; success of a response does not guarantee a recommendation for every phrase. Invalid/abstained/late responses leave normal input unchanged. Development mode intentionally displays valid low-confidence decisions with their numeric score.

The installed IPC regression also checks capability negotiation, unknown/sensitive-scope suppression, custom-term Tab commits, phrase suffix commits, and invalidation after moving the caret. These tests supply synthetic scope signals; they do not prove every real host reports its scope correctly. Reopen host applications after upgrading the TSF DLL. Loaded old clients may continue using the fallback popup until restarted.
