...
另外在 GitHub Actions Windows Server 2022 / VS2022 / v143 环境完成全量验证：[成功的构建记录](https://github.com/hetaodw/TypePick/actions/runs/35711722968)。对应代码提交为 `0133526104fc552de71bb0b9b6e6b2d3b196ff59`；随后仅补充本文档记录。
...
Rime 测试调用的是真实 `rime.dll`；推荐来源为本地 demo 固定规则。这验证了候选选择和窗口/Tab 桥接，不代表已经验证实际 TSF 安装、应用端 IPC 流程或模型效果。完整构建在云端完成，本机仍缺少 ATL；此次 CI 只保留测试记录，没有发布安装包或保存服务端二进制制品。
